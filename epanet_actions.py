...
class ProjectActions:
    ...
    def initialize_subsys(self, input_filename, hstep, pipe_index):
        log.debug('initializing subsys')
        toolkit.open(self.ph, input_filename, '', '')
        toolkit.settimeparam(self.ph, toolkit.HYDSTEP, hstep)
    ...
    def run_hydraulic_solver(self):
        ...
        toolkit.initH(self.ph, toolkit.SAVE)
    ...
    def add_leak(self):
        ...
        log.debug('initializing trigonometry tools with upstream node %s, downstream node %s and length %s' %(self.upstream_node_index, self.downstream_node_index, self.orig_prop.length))
        self.trig = TrigonometryTools(ph=self.ph, upstream_index=self.upstream_node_index, downstream_index=self.downstream_node_index, pipe_length=self.orig_prop.length)
        ...
        self.upstream_pipe_index = self.make_pipe(UP_ID, toolkit.getnodeid(self.ph, self.upstream_node_index), LEAK_ID, self.orig_prop)
        self.downstream_pipe_index = self.make_pipe(DOWN_ID, LEAK_ID, toolkit.getnodeid(self.ph, self.downstream_node_index), self.orig_prop)
    ...
    def make_pipe(self, id, start_node, end_node, properties):
        ...
        toolkit.setpipedata(self.ph, pipe_index, length = 1, diam = properties.diam, rough = properties.rough, mloss = properties.mloss)
        return pipe_index
    ...
    def move_leak_along_transect(self, leak_base, iterations):
        ...
        for i in range(iterations):
            ...
            results[1] = results[1] + 'upstream_pressure ({0}),downstream_pressure ({0}),upstream_flow ({1}),downstream_flow ({1})'.format(self.network_units.pressure, self.network_units.flow)
            if i != iterations - 1:
                results[0] = results[0] + ","
                results[1] = results[1] + ","
        ...
        for i in range(1, round(pipe_length)):
            ...
            for j in range(iterations):
                ...
                results[index] = results[index] + "{},{},{},{}".format(result_values.up_p, result_values.down_p, result_values.up_f, result_values.down_f)
                if j != iterations - 1:
    ...
    def simulate_leak(self, leak_coeff, length_before_leak, length_after_leak):
        toolkit.setpipedata(self.ph, self.upstream_pipe_index, length = length_before_leak, diam=self.orig_prop.diam, rough=self.orig_prop.rough, mloss=self.orig_prop.mloss)
        toolkit.setpipedata(self.ph, self.downstream_pipe_index, length = length_after_leak, diam=self.orig_prop.diam, rough=self.orig_prop.rough, mloss=self.orig_prop.mloss)
        ...
        return Results(self.ph, self.upstream_node_index, self.downstream_node_index, self.upstream_pipe_index, self.downstream_pipe_index)


class TrigonometryTools:

    def __init__(self, ph, upstream_index, downstream_index, pipe_length):
        upstream_node_elevation = toolkit.getnodevalue(ph, upstream_index, toolkit.ELEVATION)
        downstream_node_elevation = toolkit.getnodevalue(ph, downstream_index, toolkit.ELEVATION)
