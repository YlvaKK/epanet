...
def main():
    ...
    model_leaks(leak_iterations) #moves leak along pipe and records pressure and flow #TODO: remove magic number
...
def write_to_csv(iterations):
        ...
        for i in range(iterations):
            if (i == 0):
                header2 = header2 + 'leakage_position ({}),'.format(l_unit)
            header1 = header1 + 'leak_coeff: %s,,,,,' %(LEAK_COEFF*(i+1))
            header2 = header2 + 'upstream_pressure ({0}),downstream_pressure ({0}),upstream_flow ({1}),downstream_flow ({1}),leak_elev ({2}),'.format(p_unit, f_unit, l_unit)
